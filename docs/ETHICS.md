...
- No bulk access to PRAZ's *authenticated* procurement portal, bid documents, or supplier area — this remains off limits entirely. The one narrow exception is the public, unauthenticated "Latest Tenders" bulletin board, indexed by search engines and requiring no login, which is scraped on a rate-limited schedule to power the Procurement Knowledge Centre — see `docs/PROCUREMENT_KNOWLEDGE_CENTRE.md` for the full scope and reasoning, and `SECURITY.md` / `docs/ROADMAP.md` for the original context this decision revisits.
...
- **[2026-07-25]** — Added the Procurement Knowledge Centre: a regularly refreshed index of PRAZ's public, unauthenticated "Latest Tenders" bulletin board, used to power search, alerts, and AI-assisted answers. This revisits the original blanket "no scraping" stance now that the listing/detail pages are confirmed to be served without login and are search-engine indexed; scope remains strictly limited to that public listing (no authenticated area, no bid documents), rate-limited, and never resold. See `docs/PROCUREMENT_KNOWLEDGE_CENTRE.md` for the full rationale.
